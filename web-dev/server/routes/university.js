...
/**
 * @swagger
 * /api/university/certificates/issue:
 *   post:
 *     summary: Issue a new certificate
 *     tags: [Universities]
 ...
 *         description: Server error
 */
router.post('/certificates/issue', authenticateJWT, async (req, res) => {
  try {
    const {
      studentId,
      courseName,
    ...
    if (!studentId || !courseName || !programName || !grade || !dateOfIssue) {
    ...
    const certificateData = {
      ...
      status: 'active',
      blockchainTxId: '', // Will be updated after blockchain transaction
    };
    ...
    // Example:
    // const txId = await fabric.submitTransaction('CreateCertificate', certificate._id.toString(), JSON.stringify(certificateData));
